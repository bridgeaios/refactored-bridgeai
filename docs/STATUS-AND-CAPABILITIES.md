...
**Apply before deploy (swarm scaling, 28 risks/fixes, 5-layer topology, 5 tricks for unlimited scale, planetary-scale topology):** see **docs/DEPLOY-CHECKLIST.md**, **docs/SWARM-SCALING-RISKS-AND-FIXES.md**, **docs/SWARM-TOPOLOGY-5-LAYER.md**, **docs/SWARM-SCALE-UNLIMITED-5-TRICKS.md**, **docs/PLANETARY-SCALE-AGENT-TOPOLOGY.md**.  
**Full rundown for Marvin (agent context, APIs, state verify, run/deploy, replication, docs):** see **docs/MARVIN-FULL-RUNDOWN.md**.  
**System overview aligned with AOE/Supaco (domain map, port map, wired flow, config mismatches, known issues):** see **CLAUDE.md** (project root).
...
| **Backend API tests (pytest)** | 31/31 passed |
| **State verify** | OK (audit, twin_wall, logs excluded from Merkle; approve once if root mismatch) |
...
### H. SVG Build Agent (Spec Kit)

| Component | Purpose |
|-----------|---------|
| **svg-build-orchestrator** | Spec Kit agent for SVG system build (AGENTS.md) |
| **run-svg-build.ps1** | Health checks → bridge-svg-engine → telemetry POST |
| **POST /api/telemetry/events** | Ingest svg_build_completed; stored for Taurus |
| **GET /api/telemetry/events/svg-build** | Last build event for Taurus dashboard |
| **GET /api/svg-build/history** | Last 5 build logs from E:\A\logs |
| **Taurus core** | `C:\Users\supas\bridge-ai-os\taurus` (canonical) |
| **Omni CLI** | Not present on this host — non-blocking for Bridge Live Wall deploy |
| **BAN Live Wall** | /ban-live-wall.html — unified identity, module stack, SVG build status |

### I. Scripts & automation
