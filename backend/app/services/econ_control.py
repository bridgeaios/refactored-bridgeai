...
def _dot(w: dict[str, float], p: PriorityComponents) -> float:
    return (
        w.get("impact", 0) * p.impact
        + w.get("revenue", 0) * p.revenue
        + w.get("risk", 0) * p.risk
        + w.get("latency", 0) * p.latency
        + w.get("trust", 0) * p.trust
    )
...
class EconControlService:
    ...
    def set_reputation_getter(self, fn):
            ...
            if raw:
                try:
                    ...
                    if isinstance(data, dict):
                        self._weights = {**DEFAULT_WEIGHTS, **data}
                except Exception:
                    pass
    ...
    async def _save_weights(self, w):
        ...
        if self._memory:
            await self._memory.set(MEMORY_KEY_WEIGHTS, json.dumps(self._weights))
    ...
    def risk_budget(self, twin_id):
        trust = 0.5
        if self._reputation_getter:
            trust = max(0.1, min(1.0, self._reputation_getter(twin_id)))
    ...
    def build_p(self, task, twin_id, now_ts=None):
        ...
        for tag in tags:
            ...
            if t in impact_map:
                impact = impact_map[t]
                break
        if task_type in impact_map:
            impact = impact_map[task_type]
    ...
    async def canonical_score(self, task, twin_id, now_ts=None):
        ...
        if p.risk > meta["risk_budget"]:
            return 0.0, p, {**meta, "blocked": "risk_exceeded"}
        if meta["capital_assigned"] < meta["capital_required"]:
            return 0.0, p, {**meta, "blocked": "insufficient_capital"}
